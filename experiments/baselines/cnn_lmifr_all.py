...
class PytorchCNNLMIFR(SupervisedPytorchBaseModel):
    ...
    def train(self, X_train, Y_train, batch_size, num_epochs, data_frac):
        ...
        # Data size 1.0 num_epochs_l = [int(90/data_frac)] epsilon_elbo_l = [5.0] lagrangian_elbo_l = [1.0] lr_l = [1e-4] epsilon_adv_l = [1.0] adv_rounds_l = [10] lrl_l = [1e-3]
        # Data size 0.65, 1.0
                                        ...
                                        for i, (features, sensitive, semi_labels, labels) in enumerate(trainloader):
                                            ...
                                            if itot % self.n_adv_rounds == 0:
                                                if True:
                                                    ...
                                                    self.lagrangian.data.add_(lrl * self.lagrangian.grad.data)
    ...
    def update_adversary(self, features, sensitive, semi_labels):
        ...
        S_torch = sensitive.clone().detach().requires_grad_(True)
        Y_semi_torch = semi_labels.clone().detach().requires_grad_(True)
        self.pytorch_model(X_torch, S_torch, Y_semi_torch, self.discriminator)
