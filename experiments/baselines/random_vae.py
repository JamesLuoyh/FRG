...
class PytorchVFAE(SupervisedPytorchBaseModel):
    ...
    def train(self, X_train, Y_train, batch_size, num_epochs,data_frac):
      return
        ...
        #                     df = pd.read_csv(f'/work/pi_pgrabowicz_umass_edu/yluo/SeldonianExperimentResults/validation_performance_VAE.csv')
        #                     row = {'data_frac':data_frac, 'auc': auc, 'delta_dp': delta_DP, 'lr':lr, 'epoch': num_epochs}
        #                     print(row)
        #                     df = df.append(row, ignore_index=True)
        #                     df.to_csv(f'/work/pi_pgrabowicz_umass_edu/yluo/SeldonianExperimentResults/validation_performance_VAE.csv', index=False)
...
class VAE(Module):
    ...
    def forward(self, inputs):
        ...
        self.vae_loss = torch.zeros()
        print(self.vae_loss)
        # print(torch.softmax(y_decoded, dim=-1))
        self.pred = torch.bernoulli(self.vae_loss) # torch.softmax(y_decoded, dim=-1)[:, 1]
        self.s = s
        self.z = self.pred
        self.y_prob = self.pred
