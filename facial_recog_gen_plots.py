...
def vfae_example(
    ...
    data_fracs = [1]##1.0,0.65, 0.40, 0.25, 0.15,0.1]#0.40]#,0.40]#, 1, 0.40]#,0.65, 0.40, 0.25, 0.15,0.1]#,  0.1][1, 0.65, 0.40, 0.25, 0.15, 0.1]#[0.5] #  0.1,0.15,0.25,0.40,0.65,1 [0.01, 0.025, , 0.15, 0.5, 1]  # 0.01, 0.025, 0.06, 0.15, 0.5, #0.001,0.01,0.05,0.1,0.33,0.66,1
    # for baseline
    # epsilon 0.4. MI 1.
                    ...
                    for epochs in epochs_l:
                        for delta in delta_l:
                            ...
                            results_dir = os.path.join(results_base_dir,
                                f"ablation_cnn_vfae_mutual_information_{epsilon}_{alpha}_{alpha_lambda}_{lambda_init}_{epochs}_{delta}_{suffix}")
                            ...
                            for i in range(2):
                                plot_savename = os.path.join(
                                    results_dir, f"debug_cnn_vfae{version_subs}_mutual_information_{epsilon}_{performance_metric}_downstream_{i}.pdf"
                                )
                                plot_generator.make_plots(
                                    fontsize=12,
                                    legend_fontsize=8,
                                    performance_label=['AUC', 'Pr($\Delta_{\mathrm{DP}}$ > 0.08)'],
